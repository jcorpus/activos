...
function reg_departamento(){
  ...
  /// metodos de ajax aqui http://www.w3schools.com/jquery/ajax_ajaxsetup.asp
	$.ajax({
		url:'controller/cdepartamento.php',
		type: 'POST',
    data: {
    ...
    msj_cat += '</div>';
    document.getElementById('msj_depa').innerHTML = msj_cat;
		},
    complete: function(){
      //alert("se completo el envio");
    },
		success: function(data){
    document.getElementById('msj_depa').innerHTML = data;
		},
...
/********************************************************/
function mod_departamento(){
  if (true) {
  var id_depa2 = $("#id_depa2").val();
  ...
  $.ajax({
    ...
    msj_cat += ' <p> Enviando .....</p>'
    msj_cat += '</div>';
    document.getElementById('msj_mod_depa').innerHTML = msj_cat;
    ...
    document.getElementById('msj_mod_depa').innerHTML = data;
...
function mostrar_departamento(datos){
  var valores=datos.split("*");
  $("#id_depa2").val(valores[0]);
...
function listar_departamento(){
  ...
  $.ajax({
    ...
    success: function(resp){
      ...
      var cadena = "";
      cadena += "<table class='table table-bordered table-dataTables'  id='tabla_departamento'>";
      cadena += "<thead class=''>";
      cadena += "<tr>";
      cadena += "<th>#</th>";
      cadena += "<th>Nombre</th>";
      ...
      for(var i = 0 ; i<valores.length; i++){
        ...
        cadena += "<td><div class='btn-group'> <button type='button' class='btn btn-success ' data-toggle='modal' data-target='#myModal_modificar' onclick='mostrar_departamento("+'"'+datos_array+'"'+");'  aria-expanded='false'>Editar <span class='glyphicon glyphicon-cog'></span></button></div></td>";
        cadena += "</tr>";
      }
      cadena += "</tbody>";
      cadena += "</table>";
