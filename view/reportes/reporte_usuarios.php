...
////*************************DATOS**********************************/
    ...
    $sql = $db->query("SELECT u.usu_id, u.usu_nombre,u.usu_estado,p.nombres,p.ape_pat, p.ape_mat, p.email FROM usuario u INNER JOIN persona p ON u.usu_id = p.id_per");
...
//$pdf->Cell(ancho, alto, contenido, borde , alineación); //Alineación L Izquierda, C Centrado, R Derecha
        ...
        $pdf->Cell(26,6,'estado',1,0,'C',1);//el ultimo parametro se rellena la celda
        $pdf->Cell(30,6,'nombres',1,0,'C',1);
        $pdf->Cell(25,6,'ap pat',1,0,'C',1);
        $pdf->Cell(20,6,'ap mat',1,0,'C',1);//el ultimo parametro se rellena la celda
    ...
    while ($row = $db->recorrer($sql)) {
        ...
        $pdf->Cell(10,6,$row["usu_id"],1,0,'C');
        $pdf->Cell(25,6,$row["usu_nombre"],1,0,'C');
        $pdf->Cell(26,6,$row["usu_estado"],1,0,'C');
        $pdf->Cell(30,6,$row["nombres"],1,0,'C');
        $pdf->Cell(25,6,$row["ape_pat"],1,0,'C');
        $pdf->Cell(20,6,$row["ape_mat"],1,0,'C');
        $pdf->Cell(70,6,$row["email"],1,0,'C');
        $pdf->Cell(15,6,$row["usu_estado"],1,1,'C');
    }
    //liberando memoria
  $db->liberar($sql);
  $db->close();
