...
class PDF extends FPDF{
   ...
   function Header(){
      ...
      $this->Cell(0,35,'Reporte Lote almacen',0,1,'C');///el cero indica que la celda ocupa el ancho de la pagina
...
////*************************DATOS**********************************/
    ...
    $sql = $db->query("SELECT *, CASE alm_lote.lote_est WHEN 1 THEN 'activo' WHEN 0 THEN 'inactivo' END estado FROM alm_lote");
...
//$pdf->Cell(ancho, alto, contenido, borde , alineación); //Alineación L Izquierda, C Centrado, R Derecha
    ...
    while ($row = $db->recorrer($sql)) {
        ...
        $pdf->Cell(15,6,$row["lote_id"],1,0,'C');
        $pdf->Cell(30,6,$row["usu_id"],1,0,'C');
        $pdf->Cell(90,6,$row["lote_desc"],1,0,'C');
        $pdf->Cell(30,6,$row["lote_freg"],1,0,'C');
